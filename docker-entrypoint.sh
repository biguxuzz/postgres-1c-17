...
if [ -z "$(ls -A "$PGDATA" 2>/dev/null)" ]; then
    ...
    # Запуск initdb напрямую
    su - postgres -c "/opt/pgpro/ent-17/bin/initdb -D $PGDATA --locale-provider=icu --locale=ru_RU.UTF-8 --lc-collate=ru_RU.UTF-8 --lc-ctype=ru_RU.UTF-8 --lc-messages=ru_RU.UTF-8 --lc-monetary=ru_RU.UTF-8 --lc-numeric=ru_RU.UTF-8 --lc-time=ru_RU.UTF-8"
